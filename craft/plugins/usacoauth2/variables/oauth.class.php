...
class OauthHandler
{
    ...
    // For now, defining these here.  Should go into a config file...
    //protected $oauth_service_url = 'https://test-api.usacycling.org/oauth/v2/token';
    protected $oauth_service_url = 'https://api.usacycling.org/oauth/v2/token';
    ...
    protected function curlRequestForToken($postParams)
    {
        ...
        curl_setopt( $s, CURLOPT_URL, $this->oauth_service_url );
     ...
     * @return bool|stdClass
     */
    protected function parseRawToken($token_raw)
    {
    ...
    public function saveTokenCookie()
    {
        if ( $this->getTokenRaw() ) {
            ...
            setcookie (
                ...
                strtotime( "+1 day" ),
                "/",                    // root path
                $_SERVER['SERVER_NAME'],       // base domain
                true,                   // secure!
                true                    // http-only
                );
        }
